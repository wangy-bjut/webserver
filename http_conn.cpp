...
// 网站的根目录
const char *doc_root = "/home/wangy/webserver/resources";
...
// 修改  重置socket 上的onshort事件  确保下一次依旧可读
...
// 解析一行  判断依据 \r\n 换行符
http_conn::LINE_STATUS http_conn::process_line()
{
   char temp;
...
http_conn::HTTP_CODE http_conn::process_request_line(char *text)
{
   ...
   m_url = strpbrk(text, " \t"); // 检查出空格字符  \t前面一定有一个空格
   if (!m_url)
   {
      return BAD_REQUEST;
   }
   ...
   *m_version++ = '\0';
   ...
   m_check_state = CHECK_STATE_HEADER; // 检查状态变成 请求头
...
http_conn::HTTP_CODE http_conn::process_read()
{
   ...
   while (((m_check_state == CHECK_STATE_CONTENT) && (line_status == LINE_OK)) || ((line_status = process_line()) == LINE_OK))
   {
      // 解析到完成一行数据 或者解析到请求体
      text = get_line();
